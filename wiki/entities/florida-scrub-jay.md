---
id: florida-scrub-jay
title: Florida scrub jay
type: entity
status: draft
created: '2026-04-24'
updated: '2026-04-24'
sources:
- '[raw/books/neuroscience/The forgetting machine _ memory, perception, and the -
  Rodrigo Quian Quiroga; hoopla digital.pdf#p1-200]'
canonical: florida-scrub-jay
confidence: high
aliases:
- Aphelocoma coerulescens
tags:
- animal cognition
- memory
- bird intelligence
related:
  concepts:
  - episodic memory
  - food caching
  - future planning
  - self-awareness
  topics: []
  entities:
  - Nicky Clayton
---


# Florida scrub jay

## Summary

A bird in the crow family studied by Nicky Clayton's group at Cambridge University that demonstrates sophisticated memory capabilities. Scrub jays cache thousands of food items across many square miles and remember not just locations but also when food was hidden, whether they were observed, and can plan for future retrieval.

## Key facts

- Florida scrub jays remember not just tens or hundreds of hiding places but thousands distributed throughout many square miles around their nests. [raw/books/neuroscience/The forgetting machine _ memory, perception, and the - Rodrigo Quian Quiroga; hoopla digital.pdf#p147]
- Scrub jays remember when they hid food, realizing that after a few days a peanut is still tasty but a worm is not. [raw/books/neuroscience/The forgetting machine _ memory, perception, and the - Rodrigo Quian Quiroga; hoopla digital.pdf#p147]
- Scrub jays remember whether they were being watched when they hid food, returning later to move the food if a witness might try to steal it. [raw/books/neuroscience/The forgetting machine _ memory, perception, and - Rodrigo Quian Quiroga; hoopla digital.pdf#p147]
- Scrub jays demonstrate planning for the future by hiding food where they know they will be able to retrieve it later, not in places that will be hard to reach. [raw/books/neuroscience/The forgetting machine _ memory, perception, and the - Rodrigo Quian Quiroga; hoopla digital.pdf#p147]

## Related pages

- Broader: [[animal memory]]
- Broader: [[spatial memory]]
- Concepts: [[episodic memory]]
- Concepts: [[food caching]]
- Concepts: [[future planning]]
- Concepts: [[self-awareness]]
- Entities: [[Nicky Clayton]]

## Provenance

- Primary source: [raw/books/neuroscience/The forgetting machine _ memory, perception, and the - Rodrigo Quian Quiroga; hoopla digital.pdf#p1-200]

## Change notes

- 2026-04-24 — page created by auto ingest.
