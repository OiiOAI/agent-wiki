---
id: three-stage-artificial-consciousness-experiment
title: Three-Stage Artificial Consciousness Experiment
type: concept
status: draft
created: '2026-04-24'
updated: '2026-04-24'
sources:
- '[raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p1-378]'
confidence: high
tags:
- artificial consciousness
- engineering consciousness
- self-organisation
- free energy principle
related:
  broader:
  - artificial consciousness
  - self-evidencing systems
  narrower:
  - markov blanket
  - precision optimisation
  - free energy minimisation
  adjacent:
  - Mark Solms
  - Karl Friston
---


# Three-Stage Artificial Consciousness Experiment

## Summary

Solms describes a three-stage research programme for engineering artificial consciousness. The first stage creates an unconscious self-organising system with a Markov blanket. The second stage introduces multiple competing needs requiring embodiment. The third stage adds precision optimisation enabling flexible contextual prioritisation of needs, at which point Solms predicts consciousness will emerge.

## Key facts

- Stage 1 creates an unconscious self-organising system equipped with a Markov blanket, sensory states, active states and internal states that automatically models the world by minimising free energy. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p257]
- Stage 2 gives the system multiple needs, including the need to acquire external Gibbs free energy, making the computer hardware the system's 'body'. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p257-258]
- Stage 2 parameters include energy acquisition, thermoregulation, fatigue, pain (triggered by physical damage), anxiety, and competition with other agents. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p258]
- Stage 3 requires flexible prioritisation of multiple needs through precision optimisation on a contextual basis, where quantum of external energy varies in relation to thermoregulatory and anxiety thresholds. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p259]
- Solms predicts consciousness arises only at Stage 3 when the system can compartmentalise different error values and flexibly modulate their precision weightings. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p259]
- Stage 1 systems will have subjective values but no sentience; Stage 3 systems are predicted to be sentient. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p257,259]
- Many iterations of the system would fail and expire; successful predictive codes would be artificially propagated through successive generations. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p258-259]
- The operational criterion for voluntary activity is survival in novel unpredicted environments, which Stage 2 systems are predicted to fail at but Stage 3 systems should succeed at. [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p263]

## Inferences

- Inference: The three stages parallel biological evolution: simple self-organisation, emergence of basic drives, then emergence of flexible value-based decision-making that generates subjective experience.

## Uncertainties

- Uncertain: Whether artificial natural selection is necessary or whether the system could self-reproduce is not determined in this chunk.

## Related pages

- Broader: [[artificial consciousness]]
- Broader: [[self-evidencing systems]]
- Narrower: [[markov blanket]]
- Narrower: [[precision optimisation]]
- Narrower: [[free energy minimisation]]
- Adjacent: [[Mark Solms]]
- Adjacent: [[Karl Friston]]
- Concepts: [[sentience]]
- Concepts: [[proto-intentionality]]
- Concepts: [[hedonic valence]]
- Concepts: [[affective zombies]]
- Topics: [[artificial consciousness]]

## Provenance

- Primary source: [raw/books/neuroscience/The Hidden Spring - Mark Solms.pdf#p1-378]

## Change notes

- 2026-04-24 — page created by auto ingest.
